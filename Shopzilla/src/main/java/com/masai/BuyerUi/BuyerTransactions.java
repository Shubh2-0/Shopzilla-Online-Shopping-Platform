...
public class BuyerTransactions extends JFrame {
	...
	public BuyerTransactions() {
		...
		table.setModel(new DefaultTableModel(new Object[][] {},
				new String[] { "Return Policy", "Total Amount", "Tax Amount", "Gst Percentage", "Price",
						"Price Per Unit", "Purchase Date", "Quantity", "Buyer Name", "Buyer Id", "Product Name",
		...
		btnNewButton_2.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (s != null && e1 != null) {
					...
					if (s.isAfter(e1))
						JOptionPane.showMessageDialog(null, "From Date not should be after To Date...");

					else {
						try {
							table.setModel(DbUtils.resultSetToTableModel(
									buyerDao.getBuyerAllTransactionbyDateRange(s, e1, buyer.getBuyerUserName())));
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					if (sortingValue.equals("Total Price"))
						table.setModel(DbUtils.resultSetToTableModel(
								buyerDao.getBuyerAllTransactionByTotalPrice(buyer.getBuyerUserName())));

					else if (sortingValue.equals("Quantity"))
						table.setModel(DbUtils.resultSetToTableModel(
								buyerDao.getBuyerAllTransactionByQuantity(buyer.getBuyerUserName())));

					else if (sortingValue.equals("Purchase Date"))
						table.setModel(DbUtils.resultSetToTableModel(
								buyerDao.getBuyerAllTransactionByPurchaseDate(buyer.getBuyerUserName())));

					else
						table.setModel(DbUtils.resultSetToTableModel(
								buyerDao.getBuyerAllTransactionByGSTPercentage(buyer.getBuyerUserName())));
		...
		btnNewButton_1.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (list.indexOf(id) < 0) {
					...
					String status = transactionDao.alreadyReturnOrNot(id);
					
					if( !status.equals("NO")   ) {
						
						JOptionPane.showMessageDialog(null, "You already claim "+status+" for this transaction");
						return;
					}
					...
					if (ChronoUnit.DAYS.between(d, today) > 30) {

						JOptionPane.showMessageDialog(null,
								"We regret to inform you that your product's purchasing date falls outside the 30-day return period.\n"
					...
					returnProduct.setRefundDate(d);
					returnProduct.setTransactionId(id);
